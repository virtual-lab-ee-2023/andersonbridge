...
function Set_c()
{
	...
	{
		document.f1.R1.value= 1;
		document.f1.L1.value= parseFloat(document.getElementById('c33').value);
	}
}
function simulate_rc()
{
	if(check==1)
	{
		...
		document.f1.c333.value = ((c*(R3/R4)*((r*(R2+R4))+(R2*R4)))*1000).toPrecision(5);
		document.f1.r333.value = (((R2*R3)/R4)-r1);
		document.f1.rd33.value = ((w * (c*(R3/R4)*((r*(R2+R4))+(R2*R4))))/ (((R2*R3)/R4)-r1)).toPrecision(5);
	}
	else
	{
...
function changeImage() {
	...
	function execute_ckt()
	{
		if(check==1)
		{
			...
			var r1=[], R3=[], R4=[], v1=[], xc=[], xl=[], R1=[], R2=[], r=[], f1;
			var z1=[], z2=[], z3=[], z4=[], i2=[], i1=[], ic=[], dv=[], dvv;
			...
			r[0]=parseFloat(document.getElementById('r').value); r[1]=0;
			r1[0]=parseFloat(document.getElementById('r1').value); r1[1]=0; 
			R2[0]=parseFloat(document.getElementById('R2').value); R2[1]=0;
			xc[1]=(-1 / (w * parseFloat(document.getElementById('C').value))); xc[0]=0; 
			xl[1]=(w * ((parseFloat(document.getElementById('c33').value))/1000)); xl[0]=0; 
			v1[0]=parseFloat(document.getElementById('v1').value); v1[1]=0; 
			
			z1=add(add(xl,R1),r1);
			z2=R2;
			z3=R3;
			z4=div(mult(R4,add(r,xc)),add(add(xc,r),R4));
			i1= div(v1, add(z1,z3));
		    i2= div(v1, add(z2,z4));
			ic=mult(i2,div(R4,add(add(r,xc),R4)));
			dv=add(mult(i1,z1),(mult([-1,0], (add(mult(i2,z2),mult(ic,r))))));
			...
			document.f1.A1.value= dvv.toPrecision(6);
			document.f1.A2.value= dvv.toPrecision(6);
			perform_meter();
		}
